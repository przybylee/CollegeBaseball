...
#Save obs
#write.table(obs, file = "hitter_obs.csv", col.names = T, row.names = F, sep = ",")
obs <- read.csv("hitter_obs.csv", header = T, sep = ",")
summary(obs)
...
obs.yr <- obs[obs$YR >2012,c(1:8, 21:23)]
obs.yr$YRf <-as.factor(obs.yr$YR)
#memory.limit(size = 3676)
model1 <- lm(OPS ~ playerID + LGYR + playerID:YRf, weights = PA, data = obs.yr)
anova(model1)
model2 <- lm(OPS ~ LGYR + playerID + playerID:YRf, weights = PA, data = obs.yr)
anova(model2)
model3 <- lm(OPS ~ LGYR + playerID:YRf, weights = PA, data = obs.yr)
anova(model3)
summary(model3)
