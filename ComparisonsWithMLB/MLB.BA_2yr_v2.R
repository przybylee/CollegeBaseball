...
n <- 1
for(yr in 1872:2018){
  obs.yr <- obs[obs$YR >= yr-1 & obs$YR <= yr,c(1:8, 21:24)]
...
LEplot <- LEplot+labs(title = "League Effects on BA", x = "Year", y = "Difference from NL 2018 BA")+ scale_x_continuous(breaks=seq(1870, 2020, 10))
...
#Compare to the cramer study in the notes
Cramer <- ssn2NL.2018[ssn2NL.2018$YR<= 1979,c("LG","YR","rel.HA", "HA.pval")]
Cramer$rel.HA <- Cramer$rel.HA - Cramer$rel.HA[4]
NLCramer <- Cramer[Cramer$LG == "NL",]
ALCramer <- Cramer[Cramer$LG == "AL",]
example <- cbind(NLCramer[NLCramer$YR >1959,], ALCramer[ALCramer$YR >1959,])
example