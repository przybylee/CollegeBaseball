...
hit$RookieYR <- as.factor(hit$YR - hit$experience + 1)
...
summary(hit)
t0 <- Sys.time()
mod2 <-lmer(OPS ~ Plyr + lg.yr +(1|plyr.yr)+(1|tm.yr), REML = T, data = hit, weights = PAsc, verbose = 1)
tf <- Sys.time()
print(tf - t0)
t0 <- Sys.time()
...
print(tf - t0)
########
#Look for a pattern in the blubs for tm.yr effects
u = ranef(mod2)$tm.yr
names(u) = c("tm_blup")
#Here we consider the data from the 12 team NL era:
eraNL1 = hit[hit$LG == "NL" & hit$YR >=1892 & hit$YR <= 1899,]
teamsNL1 = unique(as.character(eraNL1$TM))
m = length(teamsNL1)
n = length(1892:1899)
blupsNL1 = data.frame(matrix(data = NA, nrow = n, ncol = m))
yrs = 1892:1899
row.names(blupsNL1) = yrs
for (tm in teamsNL1){
  for (yr in yrs){
    tmyr = paste(tm, yr)
    blupsNL1[yr,tm] <- u$tm_blup[tmyr]
  }
}
