...
summary(hit)
t0 <- Sys.time()
mxmod1 <-lmer(OPS ~ (1|Plyr) + (1|lg.yr) , REML = F, data = hit, verbose = 0)
tf <- Sys.time()
print(tf - t0)
t0 <- Sys.time()
mxmod2 <-lmer(OPS ~ (1|Plyr) + (1|lg.yr) , REML = T, data = hit, weights = 30*PAsc, verbose = 0)
tf <- Sys.time()
print(tf - t0)
t0 <- Sys.time()
mxmod3 <-lmer(OPS ~ lg.yr + (1|Plyr), REML = F , data = hit, weights = PAsc, verbose = 0)
tf <- Sys.time()
print(tf - t0)
#ss <- getME(mxmod3,c("theta","fixef"))
#mxmod3 <- update(mxmod3,start=ss,control=lmerControl(optCtrl=list(maxfun=2e4)))
#update(m1_sc,start=ss,control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
anova(mxmod1, mxmod3)
#Check for singularity in mxmod2
tt <- getME(mxmod2,"theta")
ll <- getME(mxmod2,"lower")
min(tt[ll==0])
#0.1188531 This should not be too small
#Using scaled plate appearances helps convergence a lot in the weighted models.

#################
#Here we extract the the Blups necessary to do the comparisons using models 1 and 2.
model = mxmod2
